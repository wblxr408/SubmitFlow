...
export class NiukeAdapter implements SourceAdapter {
  ...
  async normalize(raw: RawJobRecord): Promise<NormalizedJob> {
    ...
    const normalized: NiukeNormalizedJob = {
      ...
      entry_url: firstNonEmpty(
        isValidUrl(payload.redirectExternalUrl) ? payload.redirectExternalUrl : null,
        isValidUrl(payload.detail_url) ? payload.detail_url : null,
      ) ?? buildDetailUrl(Number(raw.source_job_id)),
