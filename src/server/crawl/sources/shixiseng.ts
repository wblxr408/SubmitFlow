...
const log = createLogger('crawl/shixiseng');

interface ShixisengJobItem {
  oid: string;
  job_name: string;
  company_name: string;
  city: string;
...
interface ShixisengSearchResponse {
  code: number;
  message: string;
  data: {
    list: ShixisengJobItem[];
    count: number;
    page_count: number;
  };
}
...
type ShixisengNormalizedJob = NormalizedJob & {
  entry_url: string;
  source_job_id: string;
};

export class ShixisengAdapter implements SourceAdapter {
  readonly sourceName = '实习僧';
  ...
  async discover(ctx: CrawlContext): Promise<DiscoveredItem[]> {
    ...
    for (const city of cities) {
      for (let page = 1; page <= 10; page++) {
        try {
          const pageItems = await this.fetchSearchPage(city, page);
          if (pageItems.length === 0) break;

          for (const item of pageItems) {
            const jobType = JOB_TYPE_MAP[item.job_type || ''] || '校招';
            if (jobType === '社招') continue;

            const combined = item.job_name + ' ' + item.company_name;
            if (EXCLUDE_INDUSTRY.test(combined)) continue;
            if (!INCLUDE_TECH.test(combined)) continue;

            const entryUrl = item.url || `https://www.shixiseng.com/intern/${item.oid}`;
            items.push({
              source_job_id: item.oid,
              title: cleanText(item.job_name),
              url: entryUrl,
              metadata: {
                ...item,
                entry_url: entryUrl,
          ...
          log.warn({ err, city }, 'Failed to fetch Shixiseng city page');
        }
      }
      if (items.length >= 500) break;
    }
  ...
  private async fetchSearchPage(city: string, page: number = 1): Promise<ShixisengJobItem[]> {
    ...
    const response = await axios.get<ShixisengSearchResponse>(
      `https://www.shixiseng.com/interns?${params.toString()}`,
      {
    ...
    if (response.data.code !== 0 && response.data.code !== 200) {
      throw new Error(`Shixiseng API error: ${response.data.message}`);
    }
  ...
  async fetchDetail(item: DiscoveredItem, _ctx: CrawlContext): Promise<RawJobRecord> {
    ...
    const jobItem = metadata as unknown as ShixisengJobItem & { entry_url?: string };

    let detailHtml = '';
    try {
      const resp = await axios.get(item.url || `https://www.shixiseng.com/intern/${item.source_job_id}`, {
        headers: HTTP_HEADERS,
        timeout: 20000,
      });
  ...
  async normalize(raw: RawJobRecord): Promise<NormalizedJob> {
    const payload = raw.raw_payload as unknown as ShixisengJobItem & {
      detail_html?: string;
      entry_url?: string;
    };
    ...
    if (!companyName) {
      throw new Error(`Shixiseng job ${raw.source_job_id} is missing company_name`);
    }
    if (!title) {
      throw new Error(`Shixiseng job ${raw.source_job_id} is missing job_name`);
    }

    const jdText = this.extractJdFromHtml(payload.detail_html);
    const internshipType = this.normalizeJobType(payload.job_type);
    const entryUrl = payload.entry_url || `https://www.shixiseng.com/intern/${raw.source_job_id}`;

    const normalized: ShixisengNormalizedJob = {
  ...
  private extractJdFromHtml(html: string | undefined): string | null {
    ...
    try {
      ...
      for (const selector of jdSelectors) {
        const node = $(selector);
        if (node.length > 0) {
          return cleanText(node.text()) || null;
        }
      }

      const body = $('body');
      const text = body.clone().find('script, style, nav, header, footer, .sidebar, .ad').remove().end().text();
  ...
  private normalizeJobType(jobType: string | undefined): string {
    if (!jobType) return '校招';
    return JOB_TYPE_MAP[jobType] ?? '校招';
  }

  async extractEntrypoints(job: NormalizedJob): Promise<JobEntrypointInput[]> {
    const shixiJob = job as ShixisengNormalizedJob;
    return [
      {
        entry_type: 'official',
        entry_url: shixiJob.entry_url,
        visibility: 'public',
        requires_auth: false,
        referrer_name: '实习僧',
        source_job_id: shixiJob.source_job_id,
      },
    ];
  }
